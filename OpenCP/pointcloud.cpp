	...
	*/
	...
	void PointCloudShow::warp_xyz(Mat& dest, Mat& image_, InputArray xyz_, InputArray R_, InputArray t_, InputArray k_)
	{
		...
		if (viewSW == 0)//image view
		{
            Mat matone=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(image, destImage, xyz, R, t, k, matone, matone, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(image, destImage, xyz, R, t, k, matone, matone, false, isRotationThenTranspose);
		}
		else//depth map view
		{
            ...
            Mat matone=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(dispC, destImage, xyz, R, t, k, matone, matone, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(dispC, destImage, xyz, R, t, k, matone, matone, false, isRotationThenTranspose);
		}
	...
	void PointCloudShow::warp_depth(Mat& dest, Mat& image_, Mat& srcDepth_, InputArray srcK_, InputArray srcDist_, InputArray R_, InputArray t_, InputArray destK_, InputArray destDist_)
	{
		...
		if (viewSW == 0)//image view
		{
            Mat matone=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(image, destImage, xyz, R, t, destK, destDist, matone, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(image, destImage, xyz, R, t, destK, destDist, matone, false, isRotationThenTranspose);
		}
		else//depth map view
		{
            ...
            Mat mydata=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(dispC, destImage, xyz, R, t, destK, destDist, mydata, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(dispC, destImage, xyz, R, t, destK, destDist, mydata, false, isRotationThenTranspose);
		}
	...
	void PointCloudShow::warp_depth(Mat& dest, Mat& image_, Mat& srcDepth_, float focal, InputArray R_, InputArray t_, InputArray k_)
	{
		...
		if (viewSW == 0)//image view
		{
            Mat mydata=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(image, destImage, xyz, R, t, k, mydata, mydata, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(image, destImage, xyz, R, t, k, mydata, mydata, false, isRotationThenTranspose);
		}
		else//depth map view
		{
            ...
            Mat mydata=Mat();
			if (renderOpt > 0)
                projectImagefromXYZ(dispC, destImage, xyz, R, t, k, mydata, mydata, true, isRotationThenTranspose);
			else
                projectImagefromXYZ(dispC, destImage, xyz, R, t, k, mydata, mydata, false, isRotationThenTranspose);
		}
	...
	void PointCloudShow::loopXYZ(cv::InputArray image_, cv::InputArray xyz_, cv::InputArray K_, cv::InputArray R_, cv::InputArray t_, int loopcount)
	{
		...
		if (!isInit)
		{
            ...
            Size mySize=image.size();
            look = get3DPointfromXYZ(xyz, mySize, pt);
		...
		while (key != 'q')
		{
			...
			if (isLookat)
			{
                Size mymatsize=image.size();
                Point ptdata=Point(loolatx, loolaty);
                look = get3DPointfromXYZ(xyz, mymatsize, ptdata);
			}
			...
			if (viewSW == 0)//image view
			{
                Mat mymat=Mat();
				if (renderOpt > 0)
                    projectImagefromXYZ(image, destImage, xyz, R, t, K, mymat, mymat, true, isRotationThenTranspose);
				else
                    projectImagefromXYZ(image, destImage, xyz, R, t, K, mymat, mymat, false, isRotationThenTranspose);
			}
	...
	void PointCloudShow::loopDepth(cv::InputArray image__, cv::InputArray srcDepth__, float focal, int loopcount)
	{
		...
		if (!isInit)
		{
			...
			depth2XYZ(srcDepth, focal);
            Size mydata=image.size();
            look = get3DPointfromXYZ(xyz, mydata, pt);
		...
		while (key != 'q')
		{
			...
			{
                Size mymatsize=image.size();
                Point mypoint=Point(loolatx, loolaty);
                look = get3DPointfromXYZ(xyz, mymatsize, mypoint);
			}
			lookat(look, srcview, R);
			Mat r;
			Eular2Rotation(pitch - 90.0, 0.0, yaw - 90, r);
			...
			else//depth map view
			{
                ...
                Mat mymat=Mat();
				if (renderOpt > 0)
                    projectImagefromXYZ(dispC, destImage, xyz, R, t, k, mymat, mymat, true, isRotationThenTranspose);
				else
                    projectImagefromXYZ(dispC, destImage, xyz, R, t, k, mymat, mymat, false, isRotationThenTranspose);
			}
	...
	void PointCloudShow::loopDepth(cv::InputArray image_, cv::InputArray srcDepth_, cv::InputArray image2_, cv::InputArray srcDepth2_, float focal, cv::InputArray oR_, cv::InputArray ot_, cv::InputArray K_, int loopcount)
	{
		...
		if (!isInit)
		{
			...
			depth2XYZ(srcDepth, focal);
            Size matsize=image.size();
            look = get3DPointfromXYZ(xyz, matsize, pt);
		...
		while (key != 'q')
		{
			...
			if (viewSW == 0)//image view
			{
                Mat matone=Mat();
				if (renderOpt > 0)
                    projectImagefromXYZ(image2, destImage, xyz, R, t, k, matone, matone, true, isRotationThenTranspose);
				else
                    projectImagefromXYZ(image2, destImage, xyz, R, t, k, matone, matone, false, isRotationThenTranspose);
			}
			else//depth map view
			{
	...
	void PointCloudShow::loop(Mat& image_, Mat& srcDisparity_, float disp_amp, float focal, float baseline, int loopcount)
	{
		...
		if (!isInit)
		{
            ...
            Size mtmatsize=image.size();
            look = get3DPointfromXYZ(xyz, mtmatsize, pt);
		...
		while (key != 'q')
		{
			...
			if (isLookat)
			{
                Size mysize=image.size();
                Point mypoint=Point(loolatx, loolaty);
                look = get3DPointfromXYZ(xyz, mysize, mypoint);
			}
			lookat(look, srcview, R);
			Mat r;
			Eular2Rotation(pitch - 90.0, 0.0, yaw - 90, r);
