...
void vyv_precomp_(vyv_coeffs<T> *c, T sigma, int K, T tol)
{
    ...
    __attribute__ ((aligned(16))) static const complex4c poles0[VYV_MAX_K - VYV_MIN_K + 1][5] = {
    ...
    __attribute__ ((aligned(16))) complex4c poles[VYV_MAX_K];
	double q;
    __attribute__ ((aligned(16))) double filter[VYV_MAX_K + 1];
    __attribute__ ((aligned(16))) double A[VYV_MAX_K * VYV_MAX_K], inv_A[VYV_MAX_K * VYV_MAX_K];
	int i, j, matrix_size;
...
void vyv_gaussian_conv(const vyv_coeffs<T> c, T *dest, const T *src, const int N, const int stride)
{
    ...
    init_recursive_filter(q, src, N, stride, c.filter, 0, c.filter, c.K, (T)1.0, c.tol, c.max_iter);
...
void vyv_gaussian_conv_w(const vyv_coeffs<T> c, T *dest, const T *src, const int N)
{
    ...
    __attribute__ ((aligned(16))) T q[VYV_MAX_K];
	long i;
	int m, n;
	...
	/* Handle the left boundary. */
    init_recursive_filter(q, src, N, 1, c.filter, 0, c.filter, c.K, (T)1.0, c.tol, c.max_iter);
...
{
	//const vyv_coeffs<T> c = c_;
    __attribute__ ((aligned(16))) float q[VYV_MAX_K];
	long i;
	int m, n;
	...
	/* Handle the left boundary. */
    init_recursive_filter(q, src, N, 1, c.filter, 0, c.filter, c.K, 1.f, c.tol, c.max_iter);
