...
namespace cp
{
	...
	template <class T>
    void shiftImDisp(Mat& srcim, Mat& srcdisp, Mat& destim, Mat& destdisp, double amp, double sub_gap, const int large_jump = 3, Mat& mask = sMatone, int warpInterpolationMethod = INTER_CUBIC)
	{
		Mat mask_ = mask;
    ...
    void shiftDisp(const Mat& srcdisp, Mat& destdisp, float amp, float sub_gap, const int large_jump = 3, Mat& mask = sMatone)
	{
		if (srcdisp.depth() == CV_8U)
		{
			if (destdisp.empty())destdisp = Mat::zeros(srcdisp.size(), CV_8U);
	...
	void blendLRRes(Mat& iml, Mat& imr, Mat& dispL, Mat& dispR, Mat& dest, Mat& destdisp, Mat& maskL, Mat& maskR, double a, int dth = 32)
	{
		...
		vector<uchar*> rs(21);
		{
            ...
            fillOcclusionImDisp(lref, ldref,0);
            fillOcclusionImDisp(rref, rdref,0);
			for (int i = -10; i <= 10; i++)
			{
				double move = i*0.05;
	...
	void StereoViewSynthesis::viewsynth(const Mat& srcL, const Mat& srcR, const Mat& dispL, const Mat& dispR, Mat& dest, Mat& destdisp, double alpha, int invalidvalue, double disp_amp, int disptype)
	{
		...
		else if (warpMethod == WAPR_IMG_INV)
		{
            ...
            depthfilter(temp, destdisp, matone, cvRound(abs(alpha)), disp_amp);
			...
			}
		{
			//	CalcTime t("filter");
            Mat onedata=Mat();
            depthfilter(temp, destdispR, onedata, cvRound(abs(alpha)), disp_amp);
		}
		{
			//CalcTime t("inter");
	...
	void StereoViewSynthesis::viewsynthSingleAlphaMap(Mat& src, Mat& disp, Mat& dest, Mat& destdisp, double alpha, int invalidvalue, double disp_amp, int disptype)
	{
        ...
        depthfilter(temp, destdisp, onedata, cvRound(abs(alpha)), disp_amp);
